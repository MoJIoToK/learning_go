...
func demultiplexingFunc1(dataSourceChan chan int, amount int) ([]chan int, <-chan int) {
	...
	// Так как тип у нас канала — ссылочный, то все элементы будут равны nil
	for i := range output {
	...
	go func() {
		...
		// канал-источник данных закрываем все каналы-потребители
		close(done)
...
func multiplexingFunc1(done <-chan int, channels ...chan int) <-chan int {
	var wg sync.WaitGroup
...
func main() {
	...
	consumers, done := demultiplexingFunc1(startDataSource(), 5)
	c := multiplexingFunc1(done, consumers...)
