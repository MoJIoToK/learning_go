...
func main() {
	// Вспомогательный канал, который нам потребуется для синхронизации горутины, которая будет делать
	//ориентировочные замеры времени, затраченного на принятие всех запросов.
	timeMeasuringRoutineStarted := make(chan bool)
	var wgClientsDone sync.WaitGroup
	wgClientsDone.Add(amountOfClients)

	//Функция создания клиентов. Возвращает канал с запросами от клиентов.
	getDataSource := func() chan int {
		dataSource := make(chan int)
		// Вспомогательная группа ожидания, которая нужна этому замыканию, чтобы убедиться что все клиенты
	...
	// Горутина, которая дождется завершения работы клиентов и закроет канал запросов.
	//В ней же происходит замер времени, которое требуется на получение всех запросов
	//(обратите внимание не на фактическую обработку, а на фактическое получение от клиентов всех запросов)
	go func() {
		start := time.Now()
	...
	// Запускаем горутину для замера времени и других вспомогательных операций (закрытия основного канала запросов)
